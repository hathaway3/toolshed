 ...
 * use SEEK_* throughout for better portability and readability, from Christian
 * Lesage
 *
 * Revision 1.14  2010/05/28 00:15:06  aaronwolfe
 *  corrects a bug introduced in rev 1.10 that would make the program crash,
 * from Christian Lesage
 *
...
int main(int argc, char **argv) {
  ...
  /* case-insensitive comparison of the archive suffix (.ar) 5/27/10 Christian
   * Lesage */
...
void table(FILE *fp) {
  ...
  printf("                                                    file    file   "
         "stored\n");
  printf("  file name                   ver     file date     attr    size    "
         "size\n");
  printf("----------------------------- --- ---------------- ------   -----   "
         "-----\n");
    ...
    for (fnp = fnhead; fnp; fnp = fnp->fn_link) {
      if ((patmatch(fnp->fn_name, header.a_name, TRUE) == TRUE) &&
        ...
        printf("%-29s %2d  %04d/%02d/%02d %02d:%02d %s%s %7ld %7ld\n", /*+ek+*/
               header.a_name, header.a_stat, 1900 + header.a_attr.fd_date[0],
               ...
               attrs[header.a_attr.fd_attr & 7], c4tol(header.a_attr.fd_fsize),
               header.a_size);
    }
...
int get_names(int ac, char **av, int updating) {
  ...
  while (ac--)
    if (!updating || !iswild(*av))
      ...
      *(p = buf) = '\0';
      if ((q = strrchr((r = *av++), '/'))) {
        strncpy(buf, r, 79); /* copy all to buffer			*/
        buf[79] = '\0';
      ...
      while ((dp = readdir(dirp))) {
        strncpy(r, dp->d_name, (buf + 80) - r - 1);
        r[(buf + 80) - r - 1] = '\0';
        if (patmatch(q, r, TRUE))
...
int notdot(char *p) { return (strcmp(p, ".") && strcmp(p, "..")); }
...
int iswild(char *p) { return (strchr(p, '?') || strchr(p, '*')); }
/*page*/
/*
...
int gethdr(FILE *fp, HEADER *hp) {
  ...
  if (fread(hp, (size_t)SIZEOF_HEADER, 1, fp) == 0)
    return (EOF);
...
int puthdr(FILE *fp, HEADER *hp) {
  if (fwrite(hp, (size_t)SIZEOF_HEADER, 1, fp) == 0)
    return (EOF);
...
void copy_from(FILE *ifp, FILE *ofp, HEADER *hp) {
  ...
  case PLAIN:
    while (bytes > 0) {
      n = (bytes > BUFSIZ) ? BUFSIZ : (int)bytes;
...
static char *hlpmsg[] = {
    ...
    "         s   once, suppress binary; twice, suppress all file "
    "compression\n",
