...
void writebuf(int cnt, FILE *fp) {
  if (cnt > 0) {
    unsigned char small_buf[BITS * 2];
    register UWORD *bp = buf;
    int i;

    for (i = 0; i < (cnt >> 1); i++) {
      small_buf[i * 2] = (*bp >> 8) & 0xff;
      small_buf[i * 2 + 1] = *bp & 0xff;
      bp++;
    }

    if (cnt & 1) {
      small_buf[cnt - 1] = (*bp >> 8) & 0xff;
    }

    fwrite(small_buf, cnt, 1, fp);
  }
...
WORD mask1[] = {0x0000, 0x8000, 0xc000, 0xe000, 0xf000, 0xf800, 0xfc00, 0xfe00,
                0xff00, 0xff80, 0xffc0, 0xffe0, 0xfff0, 0xfff8, 0xfffc, 0xfffe};

WORD mask2[] = {0x0000, 0x0001, 0x0003, 0x0007, 0x000f, 0x001f, 0x003f, 0x007f,
...
int readbuf(int cnt, FILE *fp) {
  ...
  if (fread(small_buf, cnt, 1, fp) != 1)
    return (EOF);

  for (i = 0; i < (cnt >> 1); i++) {
    *bp++ = (small_buf[i * 2] << 8) | small_buf[i * 2 + 1];
  }

  if (cnt & 1) {
    *bp = (small_buf[cnt - 1] << 8);
  }
#endif
