...
static int do_setattr(char **argv, char *p, int attrSetMask,
		      int attrResetMask, int quiet);
	...
	for (i = 1; i < argc; i++)
	{
		...
		else
		{
			...
			else
			{
				...
				do_setattr(argv, p, file_type, data_type,
					   quiet);
			}
			if (quiet == 0)
			{
...
static int do_setattr(char **argv, char *p, int file_type, int data_type,
		      int quiet)
{
	error_code ec = 0;
	decb_path_id path;
