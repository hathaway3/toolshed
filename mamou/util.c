...
void emit(assembler *as, int byte) {
  ...
  if (as->pass == 1) {
    ...
    if (as->code_segment_start == 1) {
      ...
      decb_header_emit(as, as->psect[as->current_psect].org,
                       as->psect[as->current_psect].size);
...
/*!
        @function f_record
        @discussion Flush one line out in Motorola S or Intel Hex format or
   binary
        @param as The assembler state structure
 */
void f_record(assembler *as) {
  ...
  if (as->output_type == OUTPUT_BINARY && as->object_output == 1) {
    ...
    } else if (as->output_type == OUTPUT_SRECORD &&
               as->object_output == 1) /* S record file */
    {
      size = 2;
...
void hexout(assembler *as, int byte) {
  if (as->object_output == 1) {
    ...
    byte = lobyte(byte);
    snprintf(tmp, 8, "%c%c", hexstr[byte >> 4], hexstr[byte & 017]);
...
void finish_outfile(assembler *as) {
  ...
  if (as->output_type ==
      OUTPUT_BINARY) /* dump the binary bytes to the object file */
  {
...
/*!
        ...
        @param i Integer to process
 */
u_int hibyte(int i) { return ((i >> 8) & 0xFF); }
...
/*!
        ...
        @param i Integer to process
 */
int loword(int i) { return i & 0xFFFF; }
...
/*!
        ...
        @param i Integer to process
 */
int hiword(int i) { return (i >> 16) & 0xFFFF; }
...
int head(char *str1, char *str2) {
  while (tolower((unsigned char)*str1) != EOS &&
         tolower((unsigned char)*str2) != EOS) {
