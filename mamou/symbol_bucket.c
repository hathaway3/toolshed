...
struct nlist *symbol_add(assembler *as, char *name, int val, int override) {
  ...
  if (strchr(name, '@') != NULL) {
    snprintf(tmp_label, MAXLAB, "_tmp%s%04X%04X", name, (int)as->use_depth,
             (int)as->current_file->num_blank_lines);
  ...
  if ((np = symbol_find(as, name, 0)) != NULL) {
    ...
    if (as->pass == 2) {
      ...
      if (np->def == val || override == 1) {
        ...
        /* 1. The value is different and we can't override -- it's a phasing
         * error. */
