...
int main(int argc, char **argv) {
  ...
  snprintf(product_name, sizeof(product_name),
           "The Mamou Assembler Version %02d.%02d", VERSION_MAJOR,
           VERSION_MINOR);

  snprintf(product_copyright, sizeof(product_copyright),
  ...
  for (j = 1; j < argc; j++) {
    if (*argv[j] == '-') {
      ...
      case 'e':
        ...
        if (tolower((unsigned char)argv[j][2]) == 'e')
          as.o_cpuclass = CPU_X9;
        break;
      ...
      case 'p':
        /* Parse only output */
        as.o_cpuclass = CPU_H6309; /* So 6309 instructions come out looking good
                                      when printed */
        as.o_do_parsing = 0;
        break;
...
int mamou_assemble(assembler *as) {
  ...
  for (as->current_filename_index = 0;
       as->current_filename_index < as->file_index;
       as->current_filename_index++) {
    ...
    if (fnlen < FNAMESIZE) {
      strncpy(root_file.file, as->file_name[as->current_filename_index],
              FNAMESIZE);
      root_file.file[FNAMESIZE - 1] = '\0';
    } else {
  ...
  if (as->num_errors == 0) {
    ...
    for (as->current_filename_index = 0;
      ...
      fnlen = strlen(as->file_name[as->current_filename_index]);
      if (fnlen < FNAMESIZE) {
        memcpy(root_file.file, as->file_name[as->current_filename_index],
               fnlen + 1);
      } else {
        printf("mamou: filename too long: %s\n",
               as->file_name[as->current_filename_index]);
  ...
  /* Added to remove an object if there were errors in the assembly - BGP
   * 2002/07/25 */
  if (as->num_errors != 0) {
    ret = 1; /* error status */
  }
...
static void mamou_initialize(assembler *as) {
  ...
  if (as->pass == 1) {
    ...
    if (as->object_name != NULL) {
      ...
      if (_coco_create(&(as->fd_object), as->object_name, FAM_READ | FAM_WRITE,
                       &fstat) != 0) {
...
void mamou_pass(assembler *as) {
   ...
   */
  while (as->current_file->end_encountered == 0 &&
         _coco_readln(as->current_file->fd, input_line, &size) == 0) {
...
/*!
        ...
        @discussion Splits the input line into label, opcode, operand and
   comment.
...
void mamou_parse_line(assembler *as, char *input_line) {
  ...
  *as->line.comment = EOS;
   ...
   */
  if (numeric(*(ptrfrm + 0)) == 1 && numeric(*(ptrfrm + 1)) == 1 &&
      numeric(*(ptrfrm + 2)) == 1 && numeric(*(ptrfrm + 3)) == 1 &&
      numeric(*(ptrfrm + 4)) == 1 && *(ptrfrm + 5) == ' ') {
    ptrfrm += 6;
    input_line += 6;
  }

  /* 3. Check to see if this is a blank line. */
  while (isspace((unsigned char)*ptrfrm))
    ptrfrm++;
  ...
  if (*ptrfrm == '*' || *ptrfrm == ';' || *ptrfrm == '#') {
    strncpy(as->line.comment, input_line, sizeof(as->line.comment));
    as->line.comment[sizeof(as->line.comment) - 1] = '\0';
  ...
  *ptrto = EOS;
  ...
  if (mne_look(as, as->line.Op, &as->line.mnemonic) == 0) {
    ...
    if ((as->line.mnemonic.type == OPCODE_PSEUDO &&
         as->line.mnemonic.opcode.pseudo->info != HAS_NO_OPERAND) ||
        (as->line.mnemonic.type == OPCODE_H6309 &&
         (as->line.mnemonic.opcode.h6309->class != INH &&
          as->line.mnemonic.opcode.h6309->class != P2INH &&
          as->line.mnemonic.opcode.h6309->class != P3INH))) {
      /* Yes, it does. */
      ptrto = as->line.operand;

      if (as->line.mnemonic.type == OPCODE_PSEUDO &&
          as->line.mnemonic.opcode.pseudo->info ==
  ...
  /* Point to the line's comment field and suck up the remainder of the line as
   * a comment. */
  ptrto = as->line.comment;
...
void process(assembler *as) {
  ...
  if (as->conditional_stack[as->conditional_stack_index] == 0) {
    ...
    if (as->line.mnemonic.type == OPCODE_PSEUDO
        //			(as->line.mnemonic.opcode.pseudo->class == IF ||
        // as->line.mnemonic.opcode.pseudo->class == ELSE ||
        // as->line.mnemonic.opcode.pseudo->class == ENDC)
    ) {
  ...
  } else if (as->line.mnemonic.type == OPCODE_UNKNOWN) // && as->pass > 1)
  {
    ...
    as->line.mnemonic.opcode.h6309->func(
        as, as->line.mnemonic.opcode.h6309->opcode);
