...
void	misc()
	...
	if (0 == (*opcode & 0x0100))
		...
		if (((*opcode & 0x00c0) == 0x0080) && acdatan())
			...
			if (((*opcode & 0x00c0) == 0x00c0) && acconn())
				{
				strmne("lea ");
				effaddn();
				incomma();
...
void	nbcd()
	...
	if (*opcode & 0x0080)
		...
		if (*opcode & 0x0038)
			{
			if (!((acconn() & acaltn()) || ((*opcode & 0x38) == 0x20)))
				{
				huh();
				goto bail;
				}
			...
			dat = getdat(1);
			for (i = 0, mask = 0x8000; i < 8; mask = mask >> 1, i++)
				if (mask & dat)
					{
					if (flag)
						strmne("/");
			...
			for (i = 0, mask = 0x0080; i < 8; mask = mask >> 1, i++)
				if (mask & dat)
					{
					if (flag)
						strmne("/");
		...
		switch (*opcode & 0x0070)
			...
			case 0x0050 :				/* link, unlk					*/
				if (*opcode & 0x0008)
					...
					{
					dat = getdat(1);
					sprintf(str, "link a%1d,#$%04X", SREG(*opcode), dat);
					}
			...
			case 0x0070 :
				if ((n = (*opcode & 0x000f)) <= 7)
					...
					if (n == 0x000a)
						{
						sprintf(str, "movec %s,%c%d  %s", s, c, reg, warning);
						strmne(str);
						}
					else
						if (n == 0x000b)
							{
							sprintf(str, "movec %c%d,%s  %s", c, reg, s, warning);
							strmne(str);
							}
						else
							huh();
...
void	moveq()
	...
	if (*opcode & 0x0100)
		...
		{
		sprintf(str, "moveq #$%02X,d%1d", DATA(*opcode), DREG(*opcode));
		strmne(str);
		}
	}

...
void	or()
	...
	if ((*opcode & 0x01f0) == 0x0100)
		...
		if (*opcode & 0x0008)
			...
			sprintf(str, "d%1d,d%1d", SREG(*opcode), DREG(*opcode));
...
void	and()
	...
	if ((*opcode & 0x0130) == 0x0100)
		...
		if (*opcode & 0x0080)
			{
			if (*opcode & 0x0008)
				sprintf(str, "exg d%1d,a%1d", DREG(*opcode), SREG(*opcode));
			else
				{
				huh();
				goto bail;
			...
			{
			if (*opcode & 0x0008)
				sprintf(str, "exg a%1d,a%1d", DREG(*opcode), SREG(*opcode));
			else
				sprintf(str, "exg d%1d,d%1d", DREG(*opcode), SREG(*opcode));
			}
