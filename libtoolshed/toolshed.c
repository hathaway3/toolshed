...
const char *EOS_PTHFUL_str =
    "The file cannot be opened because the system path table is currently full";
const char *EOS_BMODE_str =
...
error_code TSMoveFile(char *srcfile, char *dstfile) {
  ...
  if (TSIsDirectory(srcfile) == 1) {
    ...
    TSDelete(srcfile);
  } else if (TSCopyFile(srcfile, dstfile, 0, 1, 0, 0, buff, sizeof(buff)) ==
             0) {
    TSDelete(srcfile);
  }
...
error_code TSDECBFree(char *pathlist, u_int *free_granules) {
  ...
  if (decbpathlist == NULL) {
    return EOS_MF;
  }
  strcpy(decbpathlist, pathlist);

  /* 2. If a comma isn't present int he string, then add it so that the path is
