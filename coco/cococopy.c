...
static error_code CopyFile(char *srcfile, char *dstfile, int eolTranslate,
			   int rewrite);
static char *GetFilename(char *path);
static EOL_Type DetermineEOLType(char *buffer, int size);
static void NativeToOS9(char *buffer, int size, char **newBuffer,
			int *newSize);
static void OS9ToNative(char *buffer, int size, char **newBuffer,
			int *newSize);
	...
	for (i = argc - 1; i > 0; i--)
	{
		if (argv[i][0] != '-')
		{
			...
			else
			{
				...
				ec = _os9_open(&destpath, desttarget,
					       FAM_DIR | FAM_READ);
			   ...
			   int rewrite)
{
	...
	ec = _os9_create(&destpath, dstfile, mode,
			 FAP_PREAD | FAP_READ | FAP_WRITE);
	if (ec != 0)
	{
		_os9_close(path);
		return (ec);
	...
	while (_os9_gs_eof(path) == 0)
	{
		...
		if (eolTranslate == 1)
		{
			if (path->isnative == 1 && destpath->isnative == 0)
			{
				...
				NativeToOS9(buffer, size, &newBuffer,
					    &newSize);

				ec = _os9_write(destpath, newBuffer,
						&newSize);

				free(newBuffer);
			}
			else if (path->isnative == 0
				 && destpath->isnative == 1)
			{
				/* source is OS-9, destination is native */

				OS9ToNative(buffer, size, &newBuffer,
					    &newSize);
				ec = _os9_write(destpath, newBuffer,
						&newSize);
