...
const char *tokens[128] = {
    ...
    "THEN",   NULL,      "STEP",   "OFF",    "+",      "-",      "*",
    "/",      "^",       "AND",    "OR",     ">",      "=",      "<",
    "SGN",    "INT",     "ABS",    NULL,     "RND",    "SQR",    "LOG",
    "EXP",    "SIN",     "COS",    "TAN",    "PEEK",   "LEN",    "STR$",
    "VAL",    "ASC",     "CHR$",   "LEFT$",  "RIGHT$", "MID$",   "POINT",
    "VARPTR", "INKEY$",  "MEM",    "ELSE",   "PCLS",   "PSET",   "PRESET",
    "LINE",   "CIRCLE",  "PAINT",  "DRAW",   "PMODE",  "SCREEN", "COLOR",
    "PCOPY",  "PLAY",    "OPEN",   "CLOSE",  "FILES",  "CHAIN",  "SWAP",
    "WAIT",   "ERROR",   "'",      "DEF",    "LOAD",   "SAVE",   "MERGE",
    "DIR",    "EDIT",    "RENUM",  "AUTO",   "DEL",    "TRON",   "TROFF",
    "BREAK",  "HEX$",    "EOF",    "FIX",    "POS",    "STRMEM", "ATN",
    "PPOINT", "STRING$", "INSTR",  "MINVAL", "MAXVAL", "TIMER",  "ERRL",
    "ERRN",   NULL,      NULL,     NULL,     NULL,     NULL,     NULL,
    NULL,     NULL};
...
error_code _cecb_detoken(unsigned char *in_buffer, int in_size,
  ...
  *out_buffer = malloc(BLOCK_QUANTUM);
  ...
  while (value != 0 && in_pos < in_size) {
    ...
    if ((ec = _decb_buffer_sprintf(&out_pos, out_buffer, &buffer_size, "%u ",
                                   line_number)) != 0)
      return ec;
    ...
    while ((character = in_buffer[in_pos++]) != 0) {
      if (mode_character_string != 0 || mode_data_statement != 0) {
        ...
        if (character < 0x80) {
          if (character == ':') {
            if (in_buffer[in_pos] == 0x80 + 73)
              continue; /* ELSE */
            if (in_buffer[in_pos] == 0x80 + 93)
              continue; /* ' */
          }
    ...
    while (in_pos < in_size &&
        ...
        /* look for hex escape */
          ...
          if (isxdigit(in_buffer[in_pos + 2])) {
            if (isxdigit(in_buffer[in_pos + 3])) {
              ...
              (*out_buffer)[out_pos++] =
                  ahex2int(in_buffer[in_pos + 2], in_buffer[in_pos + 3]);
              in_pos += 4;
              continue;
            }
          }
      ...
      if (quote_literal + data_literal + rem_literal + var_literal == 0) {
        ...
        if (in_buffer[in_pos] == '?') {
          ...
          /* Tokenize a command */
          for (i = 0; i < 0x80; i++) {
            if (tok_strncmp(tokens[i], (char *)&(in_buffer[in_pos]),
                            in_size - in_pos) == 0) {
