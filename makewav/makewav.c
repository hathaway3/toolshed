...
int fwrite_audio_byte(int byte, FILE * output)
{
	...
	else
	{
		for (j = 0; j < 8; j++)
		{
			if (((byte >> j) & 0x01) == 0)
			{
				fwrite(buffer_1200, buffer_1200_length, 1,
				       output);
				result += buffer_1200_length;
			}
			else
			{
				fwrite(buffer_2400, buffer_2400_length, 1,
				       output);
...
int main(int argc, char **argv)
{
	...
	if (argc < 2)
	{
		...
		fprintf(stderr,
			...
			seconds);
		fprintf(stderr,
			" -s<val>    Sample rate for WAV file (default %d samples per second)\n",
			sample_rate);
		fprintf(stderr,
			" -r         Treat input file as raw binary, not an S Record file.\n");
		fprintf(stderr, " -c         Input file has DECB header\n");
		fprintf(stderr,
			" -n<string> Filename to encode in header (default: %s)\n",
			filename);
	...
	else
	{
		...
		while (fgets(linebuf, 255, srec) != NULL)
		{
			...
			if (linebuf[1] != '1')
			{
				if (verbose)
					fprintf(stderr,
						"Warning: Skipping a non S1 record\n");

				continue;
			}

			total_length +=
				(digittoint(linebuf[2]) * 16) +
	...
	{
		fprintf(stderr,
			"Unable to create buffer for S record data.\n\n");
		return -1;
	}
	...
	if (binary)
	{
		...
		{
			fprintf(stderr,
				"Could not read entire input file. (%d != 1)\n\n",
				value);
			return -1;
		}
	}
	...
	{
		/* srecord file */
		int line_data_length, program_counter = 0, line_address = 0;
		...
		while (fgets(linebuf, 255, srec) != NULL)
		{
			...
			line_address =
				(digittoint(linebuf[4]) * 4096) +
				(digittoint(linebuf[5]) * 256) +
				(digittoint(linebuf[6]) * 16) +
				digittoint(linebuf[7]);

			if (program_counter != 0)
			{
				if (program_counter + line_data_length !=
				    line_address)
				{
					fprintf(stderr,
						"Discontinus S Records not allowed.\n\n");
					return -1;
				}
			}
			else
			...
			{
				buffer[byte++] =
					(digittoint(linebuf[i]) * 16) +
					digittoint(linebuf[i + 1]);
			}
		}
		...
		{
			fprintf(stderr,
				"Data buffer not filled completely (%d != %d).\n\n",
				total_length, byte);
			return -1;
		}
	}
	...
	if (decb)
	{
		...
		{
			fprintf(stderr,
				"Error: This looks like a multi-segment DECB binary, not supported by makewav\n");
			exit(1);
		}
		VERIFY(total_length == decb_len + 10,
		       "Wrong DECB block length");
		if (start_address == 0)
		...
		if (exec_address == 0)
			exec_address =
				ubuf[8 + decb_len] * 256 + ubuf[9 + decb_len];
		buffer += 5;
		total_length -= 10;
	}
	...
	{
		fprintf(stderr,
			"Warning: Exec address is outside code segment\n");
	}
...
//      buffer_2400_length = (double)sample_rate / 2400.0;
	...
	{
		fprintf(stderr,
			"Could not allocate memory for 1200 hertz buffer\n");
		return -1;
	}
	...
	{
		fprintf(stderr,
			"Could not allocate memory for 2400 hertz buffer\n");
		return -1;
	}
	...
	unsigned char checksum =
		0 + 0x0f + Checksum_Buffer((unsigned char *) filename,
					   8) + file_type + data_type + 0 +
		Checksum_Buffer((unsigned char *) &start_address,
				2) +
	...
	if (last_block_size > 0)
	{
		unsigned char checksum = 1 + last_block_size +
			Checksum_Buffer((unsigned char *)
					&(buffer[0xff * full_blocks]),
					last_block_size);
